...
2) 质量档位使用 `Premium` / `Balanced` / `Fast`；默认 `Balanced`，仅在明确需求时上调或降档。  
3) 路由采用 Aggregator-First；fallback 链与模型系列识别规则必须按文档执行，禁止绕过或私自简化。  
4) 业务代码只使用“短名”模型 ID；平台映射在统一映射表维护，新增模型必须补齐映射。  
5) 深度研究/事实核查必须启用 MoA 多模型交叉验证。  
6) 必须记录路由轨迹（实际 provider、尝试链、失败原因）用于可观测闭环。  
7) 图片生成硬约束：最低 2K；`standard` → `2k`，`hd` → `4k`，`1k` 强制升级。  
8) API Key 名称与 Provider Base URL 必须与路由文档一致，禁止自定义命名。
...
### 1.5.8 工具调用规则（skill/plugin/mcp，强制）

工具调用前必须先盘点已安装的 plugin/skill/mcp，并优先匹配 skill。  
1) 盘点已安装：`claude-code-marketplace/plugins/`、`.claude/skills/`、`~/.agents/skills/`、`tier1-skills/skills/`、`tier8-mcp-constellation/mcp-servers.json`、MCP descriptors 目录 `/Users/mauricewen/.cursor/projects/Users-mauricewen-AI-tools/mcps/`。  
2) find skill：用 skill registry/榜单（`tier1-skills/skills/skills-registry.json`）查找适配的 skill 与触发词。  
3) 选择优先级：skill → plugin → MCP → 手工步骤。  
4) 记录选型与原因到 task_plan.md/notes.md，减少用户确认。

### 1.5.9 文件驱动记忆（Clawdbot 风格，强制）

记忆系统默认只写 Markdown 文件，不使用数据库或复杂 Schema。  
1) 长期记忆入口：`MEMORY.md`（别名 `http://MEMORY.md`），仅保留稳定事实与长期约束。  
2) 日记式增量：每天对话顺序追加到 `memory/YYYY-MM-DD.md`，只追加不覆盖。  
3) 主动沉淀触发：在上下文即将被压缩/截断前、关键决策落地后，必须写回上述文件。  
4) 轻量混合检索：语义召回用于模糊匹配，关键词检索用于精确命中，按“语义命中 + 关键词命中 + 新近性”简单打分排序。  
5) 记忆由 AI 自主管理：结构化与归档由 AI 维护，不引入外部 DB/复杂索引。  
6) 安全约束：不得写入密钥/隐私/敏感数据，必要时使用占位符。

### 1.5.10 工具箱稳定性体检（强制）

1) 任务启动前运行 `ai doctor --json` 或 `./ai-tools-launcher.sh doctor --json` 获取稳定性基线。  
2) `node_lts` 非 OK 时，安装 node@20 并设置 `AI_TOOLS_NODE_BIN` / `AI_TOOLS_NODE_LTS_BIN`。  
3) Claude 授权缺失时，执行 `claude setup-token` 完成授权（launcher 会自动触发；可用 `AI_TOOLS_CLAUDE_AUTO_SETUP=0` 关闭）。  
4) `packages` 缺失时，执行 `(cd /Users/mauricewen/AI-tools && npm install)` 或 `ai doctor --fix`。  
5) 检测到环境污染（base_url/endpoint）先执行 `ai doctor --fix` 清理。  
6) 结果写入 `notes.md`/`deliverable.md`，保证可追溯。
