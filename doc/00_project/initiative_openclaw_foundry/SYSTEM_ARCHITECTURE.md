...
| Role/job pack distribution | Foundry `web/public/packs/` plus standalone `openclaw-role-packs` snapshot | Foundry remains source worktree; production install command clones pinned tag `v2026.05.25.5` from GitHub, while standalone repo installers read local copied files by default |
