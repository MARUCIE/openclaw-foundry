...
10. Every one of the 26 cataloged packs remains reachable through either a recommendation group or browse mode. Released packs can be installed after registration; pending packs are labeled `即将上线` and cannot be installed.
11. The `定策略` recommendation path opens the `战略决策` group, which contains `strategy-roundtable-advisor` plus pending strategy packs; selecting it renders the released Strategy Roundtable Advisor card and the pending companion cards.
