...
10. Every one of the 26 cataloged packs remains reachable through either a recommendation group or browse mode. All current packs are installable after registration; `Basic`, `Enriched`, and `Certified` badges disclose maturity rather than availability.
...
9. `/packs` browse mode must show all 26 cataloged packs as live cards; `tier: "stub"` renders as `Basic`, not as disabled or missing.
