...
- 生成时间: 2026-02-13T03:14:08.447Z
- 工作目录: /Users/mauricewen
...
模型调用必须遵循单一事实源：  
`/Users/mauricewen/Library/Mobile Documents/com~apple~CloudDocs/01 知识重构/06_竞品分析/MODEL_ROUTING_ARCHITECTURE_v1.2_2026-02-02.md`  
规则：  
1) 每次模型调用必须显式标注场景类型：`chat` / `code` / `vision` / `search` / `image` / `image_edit` / `video` / `video_i2v` / `embedding` / `audio_stt` / `audio_tts` / `reranker` / `workflow`。  
2) 质量档位使用 `Premium` / `Balanced` / `Fast`；默认 `Balanced`。  
3) 路由采用 Aggregator-First；fallback 链与模型系列识别规则必须按文档执行。  
4) 业务代码只使用“短名”模型 ID；平台映射在统一映射表维护，新增模型必须补齐映射。  
5) 深度研究/事实核查启用 MoA 多模型交叉验证。  
6) 记录路由轨迹（实际 provider、尝试链、失败原因）用于可观测闭环。  
7) 图片生成硬约束：最低 2K；`standard` → `2k`，`hd` → `4k`，`1k` 强制升级。  
8) API Key 名称与 Provider Base URL 必须与路由文档一致。
...
- **HTML design hook**：`doc/` 与 `outputs/reports/` 下的 standalone HTML 必须遵守项目根 `design.md`，并通过 `npm run design:check`（已并入 `npm run build`）

---

记忆系统默认只写 Markdown 文件，不使用数据库或复杂 Schema。  
- 长期记忆入口：`MEMORY.md`（别名 `http://MEMORY.md`），仅保留稳定事实与长期约束。  
- 日记式增量：每天对话顺序追加到 `memory/YYYY-MM-DD.md`，只追加不覆盖。  
- 主动沉淀触发：在上下文即将被压缩/截断前、关键决策落地后，必须写回上述文件。  
- 轻量混合检索：语义召回用于模糊匹配，关键词检索用于精确命中，按“语义命中 + 关键词命中 + 新近性”简单打分排序。  
- 记忆由 AI 自主管理：结构化与归档由 AI 维护，不引入外部 DB/复杂索引。  
- 安全约束：不得写入密钥/隐私/敏感数据，必要时使用占位符。
...
- 工具调用前先盘点已安装的 plugin/skill/mcp  
- find skill：优先用 skill registry/榜单（`tier1-skills/skills/skills-registry.json`）查找适配技能  
- 选择优先级：skill → plugin → MCP → 手工步骤  
- 记录选型与原因到 task_plan.md/notes.md，减少确认与中断  
- 盘点入口：`claude-code-marketplace/plugins/`、`.claude/skills/`、`~/.agents/skills/`、`tier1-skills/skills/`、`tier8-mcp-constellation/mcp-servers.json`、`/Users/mauricewen/.cursor/projects/Users-mauricewen-AI-tools/mcps/`

- 任务启动前运行 `ai doctor --json` 或 `./ai-tools-launcher.sh doctor --json` 获取稳定性基线  
- `node_lts` 非 OK 时，安装 node@20 并设置 `AI_TOOLS_NODE_BIN`/`AI_TOOLS_NODE_LTS_BIN`  
- Claude 授权缺失时执行 `claude setup-token` 完成授权（launcher 默认自动触发；可用 `AI_TOOLS_CLAUDE_AUTO_SETUP=0` 关闭）  
- `packages` 缺失时执行 `(cd /Users/mauricewen/AI-tools && npm install)` 或 `ai doctor --fix`  
- 检测到 base_url/endpoint 污染时优先 `ai doctor --fix`  
- 结果写入 `notes.md`/`deliverable.md`
...
最小命令模板（按优先级）：
- Claude Chrome/MCP：`npx chrome-devtools-mcp@latest --help` + `take_screenshot(...)`
- Playwright MCP：`browser_navigate` → `browser_snapshot` → `browser_click`
- agent-browser：`agent-browser open <url>` → `agent-browser snapshot -i --json` → `agent-browser click @e1`
...
- **触发**：高风险命令/显式 `--sandbox` flag/`ai sandbox <cmd>`
- **隔离级别**：
  - L0（默认）：无隔离，读操作/查询
  - L1：Docker 容器隔离，网络受限，tmpfs + bind mount
  - L2：云沙盒（E2B/Modal），完全网络隔离，临时卷 + 自动回收
- **自动升级**：命中高风险模式（`rm -rf`/`sudo rm`/`curl | bash`/`npm publish`/`git push --force`）自动升级到 L1
- **资源配额**：CPU/内存/磁盘/超时可通过 `AI_TOOLS_SANDBOX_*` 环境变量配置
- **审计证据**：所有沙盒执行落盘到 `outputs/sandbox/<task_id>/`（commands.log/file_changes.diff/resource_usage.json）
- **配置文件**：`configs/sandbox/sandbox.default.json`（默认）/ `sandbox.local.json`（本地覆盖，gitignored）
...
<!-- PIPELINE_QUEUE_POLICY:START -->
## Pipeline Command Queue Policy

- 单一事实源：任务队列执行以 `/Users/mauricewen/AI-tools/doc/00_project/initiative_ai_tools/AI_TOOLS_PIPELINES.md` 为准。
- 执行方式：按 SOP Pipeline 顺序逐个执行；允许连续多个命令组成单次命令队列执行。
- 默认行为：不要询问，不要停下来；仅在安全/HITL 或缺失关键资源导致无法推进时才中断。
- 证据要求：所有队列动作必须落盘到 `outputs/<sop-id>/<run-id>/`，并同步记录到 `task_plan.md` 与 `notes.md`。

> 执行指令固化：以后所有命令可以连续多个命令队列进行；不要询问，不要停下来。
<!-- PIPELINE_QUEUE_POLICY:END -->
